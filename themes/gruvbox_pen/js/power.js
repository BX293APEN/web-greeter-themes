export class Power {
    ...
    async doShutdown() {
        this.showMessage("シャットダウンしています<br>コンピュータの電源を切らないでください");
        await window.wait(1000);
        window.lightdm?.shutdown();
    }
    async doRestart() {
        this.showMessage("再起動しています<br>コンピュータの電源を切らないでください");
        await window.wait(1000);
        window.lightdm?.restart();
    }
    async doHibernate() {
        this.showMessage("休止モードに移行します");
        await window.wait(1000);
        window.lightdm?.hibernate();
    }
    async doSuspend() {
        this.showMessage("スリープモード");
        await window.wait(1000);
